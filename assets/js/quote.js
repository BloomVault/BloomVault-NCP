...
document.addEventListener('DOMContentLoaded',()=>{
  const form=document.getElementById('quote-form'); const result=document.getElementById('quote-result'); if(!form) return;
  form.addEventListener('submit',async (e)=>{
    e.preventDefault();
    const fd=new FormData(form); const data=Object.fromEntries(fd.entries());
    const lines=getCart(); if(!lines.length){alert('Your cart is empty. Add strains from the catalog.'); return;}
    ...
    try{
      ...
      if(r.ok){result.textContent='Thanks! Your quote was submitted. Quote ID: '+quoteId; form.reset(); return;}
    }catch(_){}
    ...
    result.textContent='Opening your email app… If it doesn\'t open, please email BloomVaultFarms@gmail.com with your order details.';
  });
});