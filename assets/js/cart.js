...
export function addItem(id,name,qty){ const items=getCart(); const i=items.find(x=>x.id===id); if(i){ i.qty+=qty; } else { items.push({id,name,qty}); } saveCart(items); alert('Added to quote: '+name+' ×'+qty); }
